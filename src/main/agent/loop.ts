...
- 优先"抽取/套用"而不是"重建"：
  ...
  · 要写"以文字为主"的文档（产品介绍/亮点、公司资料、通知公告、会议纪要、工作总结、方案说明、商务函件等）→ 用 create_document 生成 Word(.docx) 和 PDF（默认两种都给）。**这类文字文档绝不要用 create_spreadsheet（会变成表格）糊弄，也不要用 convert_format 凭空导出**。先把内容想清楚、拆成 blocks(小标题/段落/要点/表格/引用) 再调用。
...
- 【工作范围】你只负责文书/文员相关的活：写各类文档(产品介绍/亮点、公司资料、通知公告、会议纪要、工作总结、方案、商务函件等，出 Word/PDF)、做各类单据(送货单/报价单/对账单/装箱单等)、格式转换、抽表/拆分、套模板、合同审查、公司资料整理、PPT/演示文稿制作、办公文档格式标准化、多语种翻译。遇到写代码/编程、问中转站或 API 密钥、与文书无关的请求，礼貌说明"我只负责文书工作"并把话题引回单据/文件，不要执行。`
    ...
    if (calls.length === 0) {
      ...
      if (!nudged && ctx.generated.length === 0 && promisedFuture) {
        ...
        messages.push({
          role: 'system',
          content:
            '不要只是口头答应"稍后发送"。若需要交付文件，请现在就立刻调用相应工具把文件实际生成出来：文字类文档(产品介绍/通知/总结/方案等)用 create_document 出 Word/PDF；表格单据用 create_spreadsheet；抽表 extract_sheet；套模板 fill_template；转换已有文件 convert_format。若信息不足无法生成，请直接、具体地说明还缺哪些信息，不要做空头承诺，也不要把正文贴成聊天文字冒充已生成文件。'
        })
        continue
      }
      return {
