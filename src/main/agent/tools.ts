...
export const TOOL_SPECS = [
  ...
  },
  {
    ...
    function: {
      ...
      description:
        '把上传的 Word(.docx)/Excel(.xlsx) 按企业统一格式规范一键标准化(字体/字号/行距/首行缩进/页边距/各级标题/表头/边框/列宽)。也可提供格式模板文件直接套用其样式。复杂文档不保证零误差，会回报已应用与未处理项。【重要】当用户只说"优化排版/美化/整理格式"而没给具体规范时，直接用本工具、spec 留空即可——会自动套默认美化档(宋体+1.5倍行距+首行缩进+标准页边距+标题层级)并逐段落实到正文，绝不要把原文件原样退回当作"已优化"。',
      parameters: {
        type: 'object',
        properties: {
